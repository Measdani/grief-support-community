...
export default function CreateMeetupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Meetup created successfully!' })
      setTimeout(() => router.push(`/meetups/${data.id}`), 1500)
    } catch (error) {
      console.error('Error creating meetup:', error)
      setMessage({ type: 'error', text: 'Failed to create meetup' })
    } finally {
      setSubmitting(false)
    }
  }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 flex items-center justify-center">
        <p className="text-slate-600">Checking organizer status...</p>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 py-12">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {showDisclaimerModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl shadow-lg p-8 max-w-md mx-4">
              <h2 className="text-2xl font-bold text-slate-900 mb-4">Before Creating a Gathering</h2>
              ...
              <div className="space-y-4 mb-6">
                <p className="text-slate-700 text-sm">
                  To help protect our community, creating a gathering requires identity verification through a third-party background check.
                </p>

                <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 space-y-3">
                  <p className="text-slate-700 text-sm font-medium">Please understand:</p>
                  <ul className="text-slate-700 text-sm space-y-2">
                    <li>• Background verification confirms identity only</li>
                    <li>• It does not guarantee safety</li>
                    <li>• It does not eliminate all risk</li>
                  </ul>
                </div>

                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-2">
                  <p className="text-blue-900 text-sm font-medium">By continuing, you acknowledge that:</p>
                  <ul className="text-blue-900 text-sm space-y-1">
                    <li>• Gatherings are member-organized and optional</li>
                    <li>• You choose how and where to participate</li>
                    <li>• You attend gatherings at your own discretion</li>
                    <li>• Holding Space Together does not host or supervise gatherings</li>
                  </ul>
                </div>

                <p className="text-slate-700 text-sm italic">
                  We strongly encourage meeting in public spaces and trusting your instincts at all times.
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <Link
                  href="/meetups"
                  className="block w-full px-4 py-3 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 transition font-medium text-center"
                >
                  Cancel
                </Link>
              </div>
        ...
        {/* Hide form until disclaimer is acknowledged */}
        {!hasAcknowledgedDisclaimer && showDisclaimerModal ? (
          <div className="opacity-30 pointer-events-none">
            <div className="mb-8">
              <p className="text-slate-600">Please acknowledge the safety information to continue.</p>
            </div>
          </div>
        ) : (
          <>
            {/* Header */}
            <div className="mb-8">
              <Link href="/meetups" className="text-sm text-blue-600 hover:text-blue-700 mb-2 inline-block">
                ← Back to Meetups
              </Link>
              <h1 className="text-4xl font-bold text-slate-900 mb-2">Create a Meetup</h1>
              <p className="text-slate-600">Host a support group for your community</p>
            </div>
